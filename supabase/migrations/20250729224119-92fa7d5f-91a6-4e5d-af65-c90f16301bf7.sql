...
-- Admin earnings RLS (already has some policies, enhancing them)
DROP POLICY IF EXISTS "Admins can manage all earnings" ON public.admin_earnings;
CREATE POLICY "Super admins only can manage earnings"
...
-- 4. Add missing RLS policies for unprotected tables

-- Affiliate commissions (enhance existing policies)
DROP POLICY IF EXISTS "System can manage commissions" ON public.affiliate_commissions;
CREATE POLICY "System can insert commissions"
ON public.affiliate_commissions
FOR INSERT
WITH CHECK (true);

-- Token rewards table (assuming it exists)
DO $$ 
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'token_rewards') THEN
        EXECUTE 'ALTER TABLE public.token_rewards ENABLE ROW LEVEL SECURITY';
        
        EXECUTE 'CREATE POLICY "Users can view their own token rewards"
                ON public.token_rewards
                FOR SELECT
                USING (user_id = auth.uid())';
                
        EXECUTE 'CREATE POLICY "System can manage token rewards"
                ON public.token_rewards
                FOR ALL
                USING (true)';
    END IF;
...
-- 5. Create secure password hashing function
...
BEGIN
    ...
    -- Create hash using SHA-256 with salt (in production, use bcrypt)
...
-- 7. Create security alert functions
...
-- 8. Create function to fix search_path for existing functions
...
-- 9. Invalidate existing admin sessions trigger
CREATE OR REPLACE FUNCTION public.invalidate_admin_sessions_on_password_change()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
    ...
    UPDATE public.secure_admin_sessions 
    SET is_active = false, 
        expires_at = now()
    ...
    PERFORM public.create_security_alert(
        ...
        'Password was changed for admin: ' || NEW.username,
        jsonb_build_object('admin_id', NEW.id, 'username', NEW.username)
    );
...
DROP TRIGGER IF EXISTS admin_password_change_trigger ON public.admins;
CREATE TRIGGER admin_password_change_trigger
    AFTER UPDATE OF password_hash, password ON public.admins
    FOR EACH ROW
    WHEN (OLD.password_hash IS DISTINCT FROM NEW.password_hash OR OLD.password IS DISTINCT FROM NEW.password)
    EXECUTE FUNCTION public.invalidate_admin_sessions_on_password_change();
...
-- 11. Force immediate password migration for existing admins (mark as requiring password reset)
UPDATE public.admins 
SET password_reset_token = encode(gen_random_bytes(32), 'base64url'),
...
COMMENT ON TABLE public.secure_admin_sessions IS 'Secure session management for admin users';
COMMENT ON TABLE public.admin_security_audit IS 'Audit log for all admin security-related actions';
COMMENT ON TABLE public.security_alerts IS 'Security alerts and incidents tracking';
COMMENT ON FUNCTION public.hash_password IS 'Secure password hashing with salt';
COMMENT ON FUNCTION public.authenticate_admin IS 'Secure admin authentication with session management';
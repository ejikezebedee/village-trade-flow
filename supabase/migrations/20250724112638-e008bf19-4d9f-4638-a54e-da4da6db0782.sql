-- =====================================
-- 7. SECURITY HEALTH CHECK FUNCTION
-- =====================================
...
BEGIN
  ...
  temp_result := jsonb_build_object(
    ...
    'status', CASE WHEN function_count <= 5 THEN 'pass' ELSE 'fail' END,
  ...
  IF function_count <= 5 THEN passed_checks := passed_checks + 1; END IF;
  ...
  WHERE t.table_schema = 'public' 
    ...
    AND NOT EXISTS (
      ...
      WHERE c.relname = t.table_name AND c.relrowsecurity = true
    );

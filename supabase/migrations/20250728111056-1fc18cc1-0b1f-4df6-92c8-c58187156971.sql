...
CREATE TABLE public.user_roles_progression (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL,
  current_role TEXT NOT NULL DEFAULT 'buyer',
...
INSERT INTO public.token_config (role, action_type, reward_rate) VALUES
('buyer', 'purchase', 0.01),
('seller', 'sale', 0.05),
...
BEGIN
  ...
  CASE 
    WHEN p_new_role = 'seller' AND COALESCE(current_progression.current_role, 'buyer') = 'buyer' THEN
      requirements_met := true;
    WHEN p_new_role = 'agent' AND COALESCE(current_progression.current_role, 'buyer') = 'seller' THEN
  ...
  INSERT INTO public.user_roles_progression (
    ...
    p_user_id,
    p_new_role,
    COALESCE(current_progression.current_role, 'buyer'),
    true
  );
...
AS $$
BEGIN
  INSERT INTO public.user_roles_progression (user_id, current_role)
